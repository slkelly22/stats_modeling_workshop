...
You should have already loaded the `tidyverse` package so you should have access to the `gss_cat` data that lives inside the `forcats` package. 
```{r}
glimpse(gss_cat)
```
## Activity

I want you to create a linear modeling predicting `tvhours`. Use `age` and an additional factor (your choice) as predictors. Be sure to check the levels of your factor so you can interpret the coefficients. 
